...
class InputBox extends Component {
  ...
  setTyping() {
    ...
    const typingManager =
      chat.OutputTypingManager || (chat.OutputTypingManager = new OutputTypingManager(chat.id));
  ...
  handleSendAudio = (content, file) => {
    ...
    this.sendMessage(content, true, (result) =>
      FileStore.uploadFile(result.content.audio.audio.id, result),
    );
  };
  ...
  handleSendVoiceNote = (content, file) => {
    ...
    this.sendMessage(content, true, (result) =>
      FileStore.uploadFile(result.content.voice_note.voice.id, result),
    );
  };
  ...
  render() {
    ...
    let icon = (
      <SpeedDialIcon
        open={!recordingTime && recordingReady}
        openIcon={<MicrophoneIcon />}
        icon={<SendIcon />}
      />
    );
    if (editMessageId) {
      ...
      <div className={baseClass}>
        ...
        <div className={`${baseClass}__input-wrapper`}>
          <ContentEditableInput
            ...
            forwardedRef={this.newMessageRef}
          />

          <div className={`${baseClass}__emoji-picker`}>
            <React.Suspense
              fallback={
                <Button size="small" disabled aria-label="Emoticon">
          ...
          <div className={`${baseClass}__attach-wrapper`}>
            <input
          ...
          <Button className={`${baseClass}__send`} color="primary" onClick={this.handleSubmit}>
            Send
          </Button>
        </div>
