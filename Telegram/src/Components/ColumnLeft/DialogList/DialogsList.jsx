...
import Dialog from '../../Tile/Dialog/Dialog';
...
class DialogsList extends React.Component {
  ...
  renderItem = ({ index, style }, source) => {
    ...
    return (
      <DialogListItem key={x} chatId={x} chatList={chatList} hidden={this.hiddenChats.has(x)} style={style} />
    );
  ...
  render() {
    ...
    if (chats) {
      ...
      //     />
      // ));
    } else if (chatList['@type'] === 'chatListMain') {
      dialogs = Array.from(Array(10)).map((x, index) => (
        <DialogPlaceholder key={index} index={index} />
      ));
    }
