...
class Filters extends React.Component {
  ...
  setSelection(transition = true) {
    ...
    if (filterSelection) {
      ...
      filterSelection.style.cssText = `left: ${left}px; width: ${
        item.scrollWidth - 14
      }px; ${transitionStyle}`;
    }
      ...
      this.animator = new Animator(250, [
        {
          ...
          to: clamp(
            left - scroll.offsetWidth / 2 + item.offsetWidth / 2,
            0,
  ...
  onUpdateChatFilters = (update) => {
    ...
    this.setState(
      ...
      () => {
        if (
          chatList['@type'] === 'chatListFilter' &&
          filters.findIndex((x) => x.id === chatList.chat_filter_id) === -1
        ) {
      ...
      <div ref={this.filtersRef} className="filters" onWheel={this.handleWheel}>
        <div
          ...
          className={classNames('filter', {
            'item-selected': chatList['@type'] === 'chatListMain',
          })}
