  ...
  switch (chatList['@type']) {
    ...
    case 'chatListFilter': {
      return positions.find(
        (x) =>
          x.list['@type'] === 'chatListFilter' && x.list.chat_filter_id === chatList.chat_filter_id,
      );
    }
  }
...
function getGroupChatTypingString(inputTypingManager) {
  ...
  const { size } = inputTypingManager.actions;
  if (size > 2) {
  ...
  if (size > 1) {
    let firstUser;
    let secondUser;
    for (const userId of inputTypingManager.actions.keys()) {
      if (!firstUser) {
  ...
  if (inputTypingManager.actions.size >= 1) {
    ...
    const { action } = inputTypingManager.actions.values().next().value;
    switch (action['@type']) {
...
function getChatTypingString(chatId) {
  ...
  const typingManager = ChatStore.getTypingManager(chat.id);
...
function getChatLetters(chat, t) {
  ...
  const title = chat.title || t('HiddenName');
